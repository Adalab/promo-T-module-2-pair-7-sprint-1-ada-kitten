...
function renderKittenList(kittenData) {
  ...
  for (let i = 0; i < kittenData.length; i++) {
     returnKitten += `<li class="card">
            <article>
              <img
                class="card_img"
                src= ${kittenData[i].image}
                alt="siames-cat"
              />
              <h3 class="card_title">${kittenData[i].name.toUpperCase()}</h3>
              <h4 class="card_race">${kittenData[i].race}</h4>
...
elementList.innerHTML = renderKittenList(kittenData);
